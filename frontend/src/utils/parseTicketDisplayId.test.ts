...
import { parseTicketDisplayId, MIN_TICKET_NUMBER, TICKET_DISPLAY_ID_REGEX } from './parseTicketDisplayId';

describe('parseTicketDisplayId', () => {
    describe('valid refs', () => {
        const cases: Array<{ name: string; input: string; expected: { slug: string; ticketNumber: number } }> = [
            { name: 'unpadded SLYK-4', input: 'SLYK-4', expected: { slug: 'SLYK', ticketNumber: 4 } },
            { name: 'padded leading zeros SLYK-004 normalizes to 4', input: 'SLYK-004', expected: { slug: 'SLYK', ticketNumber: 4 } },
            { name: 'large number SLYK-1000', input: 'SLYK-1000', expected: { slug: 'SLYK', ticketNumber: 1000 } },
