...
describe('useDeactivateProject', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('calls updateProject with { isActive: false } and the right slug', async () => {
    const serverUpdated = makeProject({ isActive: false });
    vi.mocked(updateProject).mockResolvedValue(serverUpdated);

    const queryClient = newQueryClient();
    const { result } = renderHook(() => useDeactivateProject(SLUG), {
    ...
    expect(updateProject).toHaveBeenCalledWith(SLUG, { isActive: false });
  });
