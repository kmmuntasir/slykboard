...
describe('DeleteTicketConfirm', () => {
    ...
    it('renders the title "Delete ticket?" when open', () => {
        render(<DeleteTicketConfirm isOpen onConfirm={vi.fn()} onCancel={vi.fn()} />);
        expect(screen.getByRole('dialog', { name: 'Delete ticket?' })).toBeInTheDocument();
    });

    it('renders nothing when isOpen is false', () => {
        render(<DeleteTicketConfirm isOpen={false} onConfirm={vi.fn()} onCancel={vi.fn()} />);
    ...
    it('disables both buttons and shows "Deleting…" while isDeleting', () => {
        render(<DeleteTicketConfirm isOpen isDeleting onConfirm={vi.fn()} onCancel={vi.fn()} />);
