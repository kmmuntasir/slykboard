...
describe('apiFetch 403 project-access handler', () => {
  ...
  it('project-scoped 403 with the access-denied message fires the handler (toast + nav source)', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValue(forbiddenResponse(PROJECT_ACCESS_DENIED));

    await expect(apiFetch('/projects/acme/members')).rejects.toBeInstanceOf(ApiClientError);
  ...
  it('project-scoped 403 with a DIFFERENT message (wrong-domain email) does NOT fire the handler', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValue(forbiddenResponse('Email domain not allowed'));

    await expect(apiFetch('/projects/acme/members/new')).rejects.toMatchObject({
      status: 403,
      code: 'FORBIDDEN',
    });
  ...
  it('no handler registered: project-scoped 403 still rejects (no throw, no crash)', async () => {
    ...
    await expect(apiFetch('/projects/acme/members')).rejects.toMatchObject({
      status: 403,
