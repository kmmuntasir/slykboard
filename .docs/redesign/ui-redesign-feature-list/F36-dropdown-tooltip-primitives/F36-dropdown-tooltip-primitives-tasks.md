...
> - **D1 — named-export compound shape** (resolves the "one component per file" rule tension via shadcn compound convention; alternative is separate files per sub-part). Default: named compound exports (matches F35).
> - **D6 — TooltipContent `bg-primary` vs `bg-popover`** (design choice: high-contrast hint bubble vs neutral popover). Default: `bg-primary text-primary-foreground` (shadcn standard hint bubble).
> - **D7 — PointerEvent polyfill in shared `test-setup.ts`** (shared-file edit affecting every test; alternative is a per-test polyfill). Default: shared polyfill in `test-setup.ts` (one-time, benefits all future Radix/pointer tests).
