...
|---|----------|--------|-----------|
| D1 | `cn()` helper | **Add `clsx` + `tailwind-merge` deps; create `frontend/src/components/ui/cn.ts`** exporting `cn(...inputs) => twMerge(clsx(inputs))` | Required for variant+size+className merge (tailwind-merge dedupes conflicts like duplicate `px-*`). shadcn convention. PRD-silent → synthesis decides. **Adds 2 deps — owner sign-off surfaced.** |
...
> **Owner sign-off needed:** **D1 — `cn()` adds deps `clsx` + `tailwind-merge`.** Surface in chat. (Alternative: minimal inline join — inferior, can't dedupe Tailwind conflicts. Not recommended.) **D2 variant→token map** and **D6 Avatar fallback chain** are synthesis decisions (PRD-silent); documented defaults are above — confirm or adjust before T2/T6 start.
