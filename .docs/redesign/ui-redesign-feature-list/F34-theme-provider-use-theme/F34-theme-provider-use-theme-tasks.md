...
> **Owner sign-off needed:** **D1 — Context vs Zustand.** Default = **Context** (rationale above). Surface prominently in chat since it diverges from the auth `useAuthStore` Zustand+persist precedent — a reviewer familiar with the auth pattern may expect Zustand. **Everything else is locked by PRD §3.2 / §1.6, decisions #2/#10, and the F33 contract.** Owner sign-off on the F34 contract (re-using `THEME_STORAGE_KEY` + `resolveInitialTheme`) was recorded 2026-06-26.
