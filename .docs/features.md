...
- [x] **F27** Project settings (rename, columns) — ✨ Feature · _deps: F08_ — DONE (T1-T3 ✅; projectService.updateProject + PATCH /:slug (admin-only 403), column ops on stable-id JSONB array, delete-non-empty → CONFLICT, delete-last → blocked (min 1), duplicate-column-id rejection (schema refine + service guard), slug permanent (not editable), ProjectColumnsManager (add/rename/reorder/delete + confirmation modal) + project name editor, useUpdateProject hook; no schema/migration; typecheck/lint/format/test/build green; live browser smoke pending)
- [x] **F28** UX polish: empty / loading / error / 404 / 403 — ⬆ Enhancement · _deps: F07_ — DONE (T1-T12 ✅; sonner toasts + global MutationCache revert funnel (meta.revertMessage), Tailwind animate-pulse skeletons (board + ticket modal), EmptyState + filtered-empty vs truly-empty, Retry + RouteErrorBoundary (react-error-boundary + QueryErrorResetBoundary), ForbiddenPage (403 via RequireRole) + NotFoundPage (404) + ticket-not-found, OfflineBanner + useOnlineStatus, query 403-retry suppressed, single error-surface (no double-toast); no schema/migration (FE-only); typecheck/lint/format/build green, FE vitest 475; live browser smoke pending)
- [x] **F29** Deployment & self-host packaging — 🚀 Deployment · _deps: all above_ — DONE (T1-T10 ✅; BE Dockerfile (node:24-bookworm-slim, non-root, tini, tsx runner — pivoted from compiled node due to ESM extensionless imports) + FE Dockerfile (nginx multi-stage SPA fallback), docker-compose.prod.yml (3-service self-host stack), .dockerignore, render.yaml (Docker web service) + vercel.json (SPA rewrite), .env.example (backend+frontend+root), migrate-on-boot (gated RUN_MIGRATIONS_ON_START, against DIRECT_DATABASE_URL) + /api/health/ready probe, docs/deployment.md; no schema/migration (packaging only); docker compose up validated — all services healthy, 0000-0012 migrate cleanly, nginx serves SPA; tsc/eslint clean; live Render/Vercel/Supabase deploy pending manual)
- [x] **F30** Human-readable ticket URLs (SLYK-NNN) — ⬆ Enhancement · _deps: F16_ — DONE (T1-T5 ✅; BE parseTicketDisplayId + getTicketByNumber + GET /api/projects/:slug/tickets/:displayId resolver (indexed, prefix-match, malformed+not-found both →404 NOT_FOUND); FE unified formatTicketId (unpadded URL SLYK-4 / padded badge SLYK-004), TicketCard emits display-ID, BoardPage resolve SLYK-NNN→UUID once (modal contract {slug,ticketId:UUID} unchanged, zero cache churn), TicketNotFound for malformed/nonexistent/old-UUID deep-links; no schema/migration (reuses (project_id,ticket_number) index); slug permanent (F27); typecheck/lint/format/build green, BE vitest 497 / FE vitest 510; live browser smoke pending)
